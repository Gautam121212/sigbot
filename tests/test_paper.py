...
def test_only_gated_models_are_traded(tmp_path):
    """Recording a forecast is not recommending a trade. Replaying every
    resolved row simulated thousands of positions nobody was told to take and
    measured cost drag on noise — it reported -48% on the real ledger while
    the gated subset returned +1.85%."""
    from sigbot import paper
    from sigbot.shadow import ShadowLedger

    path = str(tmp_path / "s.db")
    ledger = ShadowLedger(path)
    # A model with a strong record, and one that is pure noise.
    for i in range(200):
        good = ledger.record("news", f"G{i}", "BUY", 0.8, 0.02, 100.0)
        ledger.resolve(good, 104.0 if i % 10 < 8 else 96.0)
    for i in range(200):
        noise = ledger.record("crypto15m", f"N{i}", "BUY", 0.5, 0.01, 100.0)
        ledger.resolve(noise, 101.0 if i % 2 else 99.0)

    gated = paper.gated_models(path)
    assert "news" in gated
    assert "crypto15m" not in gated

    focused = paper.replay(path, models=gated)
