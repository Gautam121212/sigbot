...
def test_news_reads_as_a_card_not_an_internal_score():
    """The old format led with 'Impact score: +0.42 on a -1..+1 scale' — the
    model's own number, which means nothing to a reader."""
    from datetime import datetime, timezone
    from types import SimpleNamespace as N

    from sigbot.messenger import format_news

    article = N(title="Reliance to invest Rs 75,000 crore in new energy",
                source="Economic Times",
                published_at=datetime.now(timezone.utc))
    signal = N(asset=N(symbol="RELIANCE.NS"), side="BUY", raw_score=0.71,
               validated=False, hit_rate_lower=None, n_observed=3,
               magnitude_pct=0.018, drivers=["capex announcement"],
               articles=[article])

    text = format_news(signal)
    assert "Impact score" not in text
    assert "-1..+1" not in text
    assert "Reliance to invest" in text, "the headline is the point"
    assert "Economic Times" in text
    assert "moving it up" in text


def test_an_unvalidated_news_signal_says_the_claim_is_untested():
    from datetime import datetime, timezone
    from types import SimpleNamespace as N

    from sigbot.messenger import format_news

    article = N(title="X", source="Reuters",
                published_at=datetime.now(timezone.utc))
    text = format_news(N(asset=N(symbol="X"), side="BUY", raw_score=0.71,
                         validated=False, hit_rate_lower=None, n_observed=3,
                         magnitude_pct=0.01, drivers=[], articles=[article]))
    assert "too few to say" in text
    assert "not a probability" in text


def test_a_validated_news_signal_shows_its_worst_case():
    from datetime import datetime, timezone
    from types import SimpleNamespace as N
    ...
    article = N(title="X", source="Reuters",
                published_at=datetime.now(timezone.utc))
    text = format_news(N(asset=N(symbol="X"), side="BUY", raw_score=0.42,
                         validated=True, hit_rate_lower=0.54, n_observed=140,
                         magnitude_pct=0.01, drivers=[], articles=[article]))
    assert "140 signals like this have been checked" in text
    assert "worst case 54%" in text


def test_the_news_colour_is_the_claim_not_the_record():
    """A strong score reads green even with three checks behind it. The record
    line is what stops that being mistaken for evidence."""
    from datetime import datetime, timezone
    from types import SimpleNamespace as N
    ...
    article = N(title="X", source="Reuters",
                published_at=datetime.now(timezone.utc))
    strong = format_news(N(asset=N(symbol="X"), side="BUY", raw_score=0.71,
                           validated=False, hit_rate_lower=None, n_observed=3,
                           magnitude_pct=0.01, drivers=[], articles=[article]))
    weak = format_news(N(asset=N(symbol="X"), side="SELL", raw_score=0.35,
                         validated=False, hit_rate_lower=None, n_observed=3,
                         magnitude_pct=-0.01, drivers=[], articles=[article]))
    assert "(green)" in strong and "(amber)" in weak
    assert "too few to say" in strong, "green must still show its check count"


def test_news_matches_beyond_the_board(tmp_path, monkeypatch):
    """Matching only against the board meant a story about a company you do not
    hold was read, scored, and thrown away — most of what thirty-nine feeds
    carry, discarded before it was looked at.

    An off-board name that keeps being right builds a record the screen can act
    on at the next cycle. That is the loop the board rotation was meant to
    close."""
    import json
    ...
    (tmp_path / "universe.json").write_text(json.dumps(
        [{"symbol": "OFFBOARD.NS", "kind": "equity"}]))

    scanned, _note = _load_universe()
    on_board = {a.symbol for a in POOL}
    assert scanned is not None
    extra = [a for a in scanned if a.symbol not in on_board]
    assert extra, "the scanned pool must add names the board does not hold"


def test_the_news_model_is_given_more_than_the_board(monkeypatch, tmp_path):
    """Behaviour, not prose. An earlier version asserted on comment text, which
    broke the moment the comment wrapped across a line — a test that fails for
    ...
    class _Model:
        def __init__(self, universe, *_a, **_kw):
    ...
    monkeypatch.setattr(runner, "NewsModel", _Model)
    monkeypatch.setattr(runner, "RSSProvider", _Provider)

    class _Cap:
        def send(self, _text):
            pass

    runner.run_news(messenger=_Cap())

    symbols = {a.symbol for a in seen["universe"]}
    assert "OFFBOARD.NS" in symbols, (
        "a story about an asset you could hold must be scorable even when the "
        "board does not hold it")


def test_news_forecasts_carry_an_entry_price():
    """`run_resolve` skips every row where entry is None, so news signals were
    recorded and then sat unresolvable forever. That is why the news model read
    "0 checked" after a week of firing — not because nothing cleared the
    threshold, but because nothing it recorded could ever come due."""
...
def test_scanned_universe_rows_become_real_assets(monkeypatch, tmp_path):
    """The matcher calls asset.match_terms(). A SimpleNamespace without it
    crashed the whole news job — the widening shipped without ever running the
    code path it widened."""
    import json
    ...
    monkeypatch.chdir(tmp_path)
    (tmp_path / "universe.json").write_text(json.dumps(
        [{"symbol": "OFFBOARD.NS", "name": "Offboard Industries",
          "kind": "equity"}]))
    ...
    monkeypatch.setattr(runner, "NewsModel", _Model)
    monkeypatch.setattr(runner, "RSSProvider", _Provider)
    runner.run_news(messenger=_Capture())
