...
def test_standalone_inlines_the_data(tmp_path, seeded):
    ...
    app = tmp_path / "app"
    app.mkdir()
    # app/index.html is a source file, not a disposable sample: build_standalone
    # inlines the export into its slot. Advice to delete it was wrong.
    source = Path(__file__).resolve().parents[1] / "app" / "index.html"
    assert source.exists(), (
        "app/index.html is missing. It is a template build_standalone needs, "
        "not a sample — restore it from the package.")
    shutil.copy(source, app / "index.html")
...
def test_every_model_has_an_accent_and_a_mechanism():
    """A model in MODEL_META without an entry in ACCENTS raised a KeyError deep
    inside build_export — twenty-four tests away from its cause. This fails at
    the definition instead."""
    from sigbot.export_app import ACCENTS, MODEL_META
    from sigbot.report import MECHANISM

    for model_id in MODEL_META:
        assert model_id in ACCENTS, f"{model_id} has no accent colour"
        assert model_id in MECHANISM, f"{model_id} has no plain-English "\
