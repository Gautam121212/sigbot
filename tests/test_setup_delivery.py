...
def test_a_message_is_split_but_never_indefinitely(env, monkeypatch):
    """Forty-two notifications from one opportunity scan is not delivery, it is
    a denial of service on your attention — and it is exactly what "split
    whatever you are given" produces once the feed count goes from five to
    thirty-nine."""
    from sigbot.messenger import TelegramMessenger

    tm = TelegramMessenger()
    huge = "\n\n".join(f"card {i} " + "x" * 3000 for i in range(50))
    parts = tm._chunks(huge)

    assert len(parts) <= tm.MAX_PARTS
    assert "not sent" in parts[-1]
    assert "outbox.log" in parts[-1], "the full text must remain findable"


def test_the_opportunity_digest_shows_a_handful_not_everything():
    """A digest nobody reads is worse than a short one. The rest stay on the
    Ideas tab, where they can be browsed rather than pushed."""
    from sigbot.plain_opportunities import render_plain
    from sigbot.venture import RUBRIC
    from sigbot.venture_news import NewsCandidate, OpportunityScore
    from datetime import datetime, timezone

    scores = {k: 5 for k in RUBRIC}
    many = [NewsCandidate(
        opportunity=OpportunityScore(title=f"Idea {i}", thesis="x", trigger="n",
                                     scores=scores, notes={}, sources=["Reuters"],
                                     created_at=datetime.now(timezone.utc)),
        gap_type="supply_chain_shift", needs_enrichment=[]) for i in range(30)]

    text = render_plain([], many)
    assert text.count("SUPPLY CHAIN CHANGE") <= 6
    assert "the rest are on the Ideas tab" in text
