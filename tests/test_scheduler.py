...
def test_ship_script_is_valid_and_gated():
    """`npm run github` must refuse to push broken code. A green Actions run on
    a break is worse than a red one — it publishes the break to the live
    site."""
    import subprocess
    from pathlib import Path

    script = Path(__file__).resolve().parents[1] / "scripts" / "ship.sh"
    assert script.exists(), "scripts/ship.sh is missing"
    subprocess.run(["bash", "-n", str(script)], check=True)

    text = script.read_text()
    assert text.startswith("#!/usr/bin/env bash")
    assert "set -euo pipefail" in text
    # Match the real invocations, not any occurrence of the words: the
    # script's own help text mentions `git commit`, and a substring search
    # found that first and failed a correct script.
    commit = text.index('git commit -m "$MSG"')
    assert text.index("python -m pytest") < commit, (
...
def test_package_json_exposes_the_one_command():
    import json
    from pathlib import Path
    ...
    assert scripts["github"] == "bash scripts/ship.sh"
