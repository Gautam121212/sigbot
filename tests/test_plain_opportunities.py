...
def test_weak_sourcing_is_called_out():
    ...
    # Corroborated sourcing now reaches GREEN. It previously could not: the
    # top band was AMBER, so the scale had no way to say "as complete as
    # press coverage gets".
    assert strong.colour == GREEN
...
def test_thesis_colour_bands_discriminate():
    """Two bands with GREEN unreachable meant every card rendered grey and the
    colour carried no information — the same failure shape as a tier gate set
    above what the metric can produce."""
    from types import SimpleNamespace as N

    from sigbot.plain_opportunities import AMBER, GREEN, GREY, from_thesis

    def colour_at(strength):
        return from_thesis(N(subject="X Corp", claim="A claim about X.",
                             source_strength=strength, category="",
                             must_be_true=[], articles=[])).colour

    assert colour_at(0.80) == GREEN
    assert colour_at(0.60) == AMBER
    assert colour_at(0.30) == GREY
