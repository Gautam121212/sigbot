...
| **B36** | **Follow-on moves has one resolved check in total.** The model is effectively not running — its links fire but almost never clear the gates. | C2 | Needs investigation: are the 2σ/edge gates set above what the link set can produce (a C9 in the making)? |
