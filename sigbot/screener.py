...
def score_asset(bars: pd.DataFrame, symbol: str, kind: str,
    ...
    # An asset that cannot move cannot answer. A stablecoin scores movability 0
    # by construction — it is pegged — and one that also fails to react or link
    # to anything has nothing measurable about it at all. Scoring such a thing
    # 18/100 and letting it hold a slot wastes a hundredth of the board and a
    # daily download forever.
    if (res.scores.get("movability", 10) <= 1
            and res.scores.get("reactivity", 10) <= 1
            and res.scores.get("linkage", 10) <= 1):
        res.usable = False
        res.reject_reason = ("it barely moves, does not react to events, and "
                             "links to nothing — there is no question it could "
                             "answer, however long it is watched")
    return res
