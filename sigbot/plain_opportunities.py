...
def from_thesis(card) -> PlainCard:
    ...
    """
    # Three bands, because two did not discriminate. The old split was AMBER
    # at >=0.75 and GREY below, with GREEN unreachable for a thesis at all.
    # Observed strengths run 40-70%, so in practice every card came out grey
    # and the colour carried no information — the same failure as a tier gate
    # set above what the metric can produce.
    strength = getattr(card, "source_strength", 0.0) or 0.0
    if strength >= 0.75:
        colour, verdict = GREEN, ("Corroborated by outlets with a record. The "
                                  "case is as complete as press coverage gets "
                                  "— which is still not a forecast, and has "
                                  "never been scored against outcomes.")
    elif strength >= 0.55:
        colour, verdict = AMBER, (f"Reported at {strength:.0%} sourcing "
                                  "strength. Enough to be worth reading, not "
                                  "enough to lean on. This model has never "
                                  "been scored against outcomes.")
    else:
