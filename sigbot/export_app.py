...
ACCENTS = {"news": "teal", "daily": "purple", "contagion": "red",
           "crypto15m": "amber", "opportunity": "green", "setups": "green", "stocks": "green"}
...
TARGET_CHECKS = {
    ...
    "setups": 120,
    # Wide scan across the whole universe, both tiers recorded, so this
    # accumulates far faster than the board-limited models ever did.
    "stocks": 800,
}
...
def _next_tier_above(tier):
    ...
    Ordered weakest to strongest so "next" means the nearest achievable
    milestone. Aiming every estimate at TRADE reports "not reachable" for a
    model sitting one step below it — true, and useless.
    """
    ladder = (Tier.SILENT, Tier.WATCH, Tier.CAUTION, Tier.TRADE)
    try:
        i = ladder.index(tier)
    except ValueError:
        return Tier.WATCH
...
def build_export(db_path: str = "shadow.db", patterns_path: str = "patterns.db",
    ...
    for model_id, (name, subtitle) in MODEL_META.items():
        ...
        # The null this model's metric actually has. Judging a cost-filtered
        # hit rate against 0.50 made every tier unreachable.
        null = ledger.empirical_null(model_id)
        tier, _ = classify_tier(n, (rate * n) if n else 0.0, null=null)
        next_tier = _next_tier_above(tier)
        need = (observations_needed(next_tier, rate, null=null)
                if n and rate and next_tier else None)
        # "N more checks" is unusable; "about D days" can be planned around.
        # Measured from the ledger's observed accumulation rate, not from the
        # cron schedule, because a job that skips runs accumulates slower.
        per_day = ledger.checks_per_day(model_id)
        if need and per_day and need > n:
            days = (need - n) / per_day
            label = next_tier.value if next_tier else "the next tier"
            ready_in = (f"{label} in about {days:.0f} day(s) at this rate"
                        if days >= 1 else f"{label} within a day at this rate")
        elif need and need <= n:
            ready_in = "enough checks already — the hit rate is what is short"
        elif not next_tier:
            ready_in = "already at the top tier"
        elif n:
            ready_in = f"{next_tier.value} not reachable at this hit rate"
        else:
        ...
        intake = (f"{flow[0]:,.0f} item(s) a day reach it, "
                  f"{flow[1]:,.1f} clear its bar"
                  if flow else "not enough runs logged to measure intake")
