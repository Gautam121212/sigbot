...
def prove_all() -> list[Proof]:
    ...
    # TIER_RULES now carry a MARGIN over the null, not an absolute rate, so
    # the gate has to be proved at the null it will actually face. 0.50 is the
    # worst case (an unfiltered metric); a cost-filtered one has a lower null
    # and therefore an easier absolute bar, so proving the worst case proves
    # them all.
    out = [prove(f"tier {r.tier.value}", r.min_observations,
                 0.50 + r.min_lower_bound)
           for r in TIER_RULES]
