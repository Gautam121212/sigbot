...
def check_schedule() -> Check:
    ...
    # Two ways to be automated, and this only knew about the older one. It
    # reported "not scheduled" at a coordinator that had been running for days
    # and told you to install the daily job alongside it — which would have
    # given you two processes writing one ledger and duplicate reports.
    agents = Path.home() / "Library" / "LaunchAgents"
    coordinator = agents / "com.sigbot.coordinator.plist"
    daily = agents / "com.sigbot.daily.plist"

    try:
        out = subprocess.run(["launchctl", "list"], capture_output=True,
                             text=True, timeout=10).stdout
    except Exception as exc:  # noqa: BLE001  # handled: returns a Check saying launchctl could not be asked, which is not the same as unscheduled
        return Check("Automation", WARN,
                     f"could not ask launchctl ({type(exc).__name__}) — that is "
                     "not the same as 'not scheduled'",
                     "run: launchctl list | grep sigbot")

    running_coordinator = "com.sigbot.coordinator" in out
    running_daily = "com.sigbot.daily" in out

    if running_coordinator and running_daily:
        return Check("Automation", WARN,
                     "both the coordinator and the old daily job are loaded — "
                     "two processes writing one ledger, and every report sent "
                     "twice",
                     "run: bash scripts/schedule.sh remove")
    if running_coordinator:
        return Check("Automation", OK,
                     "the coordinator is running — all eight jobs on their own "
                     "schedules")
    if running_daily:
        return Check("Automation", OK,
                     "the old daily job is scheduled. It fires once and exits, "
                     "so nothing resolves between runs",
                     "for continuous operation: bash scripts/autostart.sh")
    if coordinator.exists() or daily.exists():
        return Check("Automation", WARN,
                     "a schedule exists but nothing is loaded",
                     "run: bash scripts/autostart.sh remove && "
