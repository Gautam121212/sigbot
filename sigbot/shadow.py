...
class ShadowLedger:
    ...
    def tier(self, model: str, symbol: str, conf: float = 0.90) -> tuple[Tier, float, int]:
        ...
        tier, lower = classify_tier(n, hits, conf,
                                    null=self.empirical_null(model))
        return tier, lower, n

    def intake_per_day(self, job: str) -> tuple[float, float] | None:
        """(items seen per day, items that cleared the bar per day) for a job.

        "How much is coming in" and "how much survives" are different
        questions, and only the pair is informative: a scanner that reads
        3,000 stories a day and passes two is behaving correctly, while the
        same two out of five would mean the intake had collapsed.
        """
        with closing(sqlite3.connect(self.path)) as con:
            rows = con.execute(
                "SELECT considered, signals, ran_at FROM runs "
                "WHERE job=? ORDER BY ran_at", (job,)).fetchall()
        if len(rows) < 2:
            return None
        try:
            first = datetime.fromisoformat(str(rows[0][2]))
            last = datetime.fromisoformat(str(rows[-1][2]))
        except ValueError:
            return None
        days = max((last - first).total_seconds() / 86400.0, 0.5)
        seen = sum(int(r[0] or 0) for r in rows)
        passed = sum(int(r[1] or 0) for r in rows)
        return seen / days, passed / days

    def checks_per_day(self, model: str) -> float | None:
        """How fast this model actually accumulates scored checks.

        Needed to turn "X more checks" into "about Y days", which is the only
        form of that number a person can plan around. Measured from the
        ledger rather than from the schedule, because a job that skips runs
        accumulates slower than its cron implies and the honest estimate is
        the observed one.
        """
        with closing(sqlite3.connect(self.path)) as con:
            row = con.execute(
                ...
                (model,)).fetchone()
        n = int(row[0] or 0)
        if n < 10 or not row[1] or not row[2]:
            return None
        try:
            first = datetime.fromisoformat(str(row[1]))
            last = datetime.fromisoformat(str(row[2]))
        except ValueError:
            return None
        days = max((last - first).total_seconds() / 86400.0, 0.5)
        return n / days

    def empirical_null(self, model: str, cost_pct: float = 0.0015) -> float:
        """The rate a coin flip would score against this model's hit metric.

        `hit` requires the direction to be right AND the move to clear trading
        costs, so chance does not score 0.50 — it scores 0.50 x P(the move
        cleared costs), which is a property of the assets and horizon, not of
        the model. Measuring it from the model's own resolved rows is the only
        way the tier thresholds mean anything.

        Falls back to 0.50 when there is too little history to measure, which
        is the strictest available assumption rather than a flattering one.
        """
        with closing(sqlite3.connect(self.path)) as con:
            rows = con.execute(
                "SELECT entry_price, exit_price FROM predictions "
                "WHERE model=? AND hit IS NOT NULL AND entry_price>0 "
                "AND exit_price>0", (model,)).fetchall()
        if len(rows) < 30:
            return 0.50
        movers = sum(1 for entry, exit_ in rows
                     if abs(exit_ / entry - 1.0) > cost_pct)
        return 0.5 * (movers / len(rows))
