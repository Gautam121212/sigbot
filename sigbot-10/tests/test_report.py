...
def test_every_asset_has_a_colour_and_a_reason(board):
    for a in board[1]["board"]["assets"]:
        assert a["flag"] in ("GREEN", "AMBER", "RED")
        assert a["colour"].startswith("#") and a["reason"]
        assert a["flag_label"] in ("Ready to trade", "Risky — watch only", "Do not trade")


def test_colour_follows_the_record_not_an_opinion(board):
    by_sym = {a["symbol"]: a for a in board[1]["board"]["assets"]}
    for sym in ("NVDA", "SPY"):
    ...
    assert by_sym["SPY"]["flag"] == "AMBER", "20 checks is too few for any verdict"
    assert "Too early" in by_sym["SPY"]["reason"]


def test_unchecked_assets_are_amber_not_green(board):
    unchecked = [a for a in board[1]["board"]["assets"] if a["n"] == 0]
    assert unchecked, "most of a fresh board should be unchecked"
    assert all(a["flag"] == "AMBER" for a in unchecked)
