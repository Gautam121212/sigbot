...
def _band(checked: int, per_day: float, brier: float | None) -> tuple[str, str, int | None]:
    if per_day <= 0:
    ...
    if checked >= CHECKS_FOR_VERDICT:
        return GREEN, ("Enough checks to judge. Whether the answer is good is "
                       "the Board's question, not this one."), 0
    ...
    try:
        with sqlite3.connect(db_path) as con:
            ...
            for model in models:
                ...
                colour, verdict, days = _band(checked, per_day, brier)
                out.models.append(ModelProgress(
                    model, made, checked, hits, brier, per_day, colour,
                    verdict, days))
    ...
    for model in progress.models:
        ...
        if model.checked >= CHECKS_FOR_CALIBRATION:
            ...
            lines.append(f"  {model.hits}/{model.checked} right so far, worst "
                         f"case {low:.0%} — too few to mean anything yet.")
        lines.append("")
