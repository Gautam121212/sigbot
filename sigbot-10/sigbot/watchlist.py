...
class Flag(str, Enum):
    ...
    @property
    def label(self) -> str:
        return {Flag.GREEN: "Ready to trade",
                Flag.AMBER: "Risky — watch only",
                Flag.RED: "Do not trade"}[self]

    @property
    def colour(self) -> str:
        return {Flag.GREEN: "#00e676", Flag.AMBER: "#ffd93d", Flag.RED: "#ff6b6b"}[self]
...
def classify(n: int, hits: float, rules: Rules = RULES) -> tuple[Flag, str]:
    ...
    if n < rules.amber_min_n:
        return Flag.AMBER, (f"Only {n} checks so far. Too early to say anything — "
                            f"needs {rules.amber_min_n} before the colour means much.")
...
class Watchlist:
    ...
    def review(self, records: dict[str, tuple[int, float]],
        ...
        order = {Flag.RED: 0, Flag.GREEN: 1, Flag.AMBER: 2}
