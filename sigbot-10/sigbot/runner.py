...
def run_news(messenger=None, settings=SETTINGS, hours: int = 12) -> None:
    ...
    model = NewsModel(board_assets(settings), LexiconClassifier(),
    ...
    for s in signals:
        ledger.record("news", s.asset.symbol, s.side, s.raw_score, None, None, 24)
...
def _write_opportunity_store(cards, candidates,
    ...
    try:
        Path(path).write_text(
            json.dumps(build_opportunities(cards, candidates), indent=1),
            encoding="utf-8")
    except Exception as exc:  # noqa: BLE001
...
def run_opportunities(messenger=None, settings=SETTINGS, hours: int = 24) -> None:
    ...
    _write_opportunity_store(cards, candidates)
    ...
    if fresh_listings:
        body += "\n\n" + "\n\n".join(x.render_text() for x in fresh_listings[:3])
...
def run_resolve(market=None, settings=SETTINGS) -> None:
    """Score every prediction whose 24h window has closed."""
    market = market or YahooProvider()
    ledger = ShadowLedger(settings.shadow_db)
    end = (datetime.now(timezone.utc) + timedelta(days=1)).strftime("%Y-%m-%d")
    cache: dict[str, float] = {}
    resolved = 0
    for pred_id, symbol, _side, entry in ledger.due():
        if entry is None:
            continue
        try:
            if symbol not in cache:
                cache[symbol] = float(
                    market.history(symbol, settings.history_start, end)["close"].iloc[-1]
                )
...
def run_cycle(messenger=None, market=None, settings=SETTINGS,
    ...
    if rescreen:
        try:
            ...
            loaded = report("cycle_screen", len(POOL))
            if loaded.skipped:
                print(loaded.line())
